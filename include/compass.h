...
/**
   Copy IMU compass model parameters into global structures so they can be used
   by read_compass().

   This function can be used as an alternative to imu_calibrate_compass() (assuming
   you know the right parameters)

 */

void imu_set_compass_params(float meanX, float meanY, float meanZ,
...
/**
   Read a calibrated compass heading.

   Assumption: the compass parameters have already been set.

   @return the compass heading in radians
*/


float imu_read_compass(void){
  float mag[3];
